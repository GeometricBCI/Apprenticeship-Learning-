...
class Gridworld(object):
    ...
    def initialize_random_policy(self, state_int):
        
        sx, sy = self.int_to_point(state_int)

        if sx == 0 and sy == 0:
            return rn.randint(0,2)
        if sx == self.grid_size-1 and sy == self.grid_size-1:
            return rn.randint(2,4)
        if sx == 0 and sy == self.grid_size-1:
            return rn.choice([0,3])
        if sx == self.grid_size-1 and sy == 0:
            return rn.choice([1, 2])

        if (sx < self.grid_size and 0 < sx) and (sy < self.grid_size and 0< sy):
            return rn.randint(0,4)
        if sx < self.grid_size and 0 < sx and sy == self.grid_size-1:
            return rn.choice([0, 2, 3])
        if sx < self.grid_size and 0< sx and sy == 0: 
            return rn.choice([0, 1, 2])
        if sy < self.grid_size-1 and sy >0 and sx == self.grid_size-1:
            return rn.choice([0, 1, 3])
        if sy < self.grid_size-1 and sy >0 and sx == 0:   
            return rn.choice([1, 2, 3])
        raise ValueError("Unexpected state.")
        ...
        for _ in range(n_trajectories):
            ...
            for _ in range(trajectory_length):
                if rn.random() < self.wind:
                    ...
                    # Follow the given policy.
                    state_int_0 = self.point_to_int((sx, sy))
                    pick_action = rn.choice([0,1,2,3], p = [Q_table[state_int_0, a] for a in range(self.n_actions)])
                    action = self.actions[pick_action]
